...
def generate_quiz(creater, topic):
    try:
        # Define the prompt template (This was missing!)
        prompt_template = """
            Generate a quiz on the topic of {topic} with atleat 4 options.  Return *only* the following JSON. Do not include any other text, explanations, or code blocks. The JSON *must* strictly adhere to this format:
            {{
            "creator":"{creater}"
            "topic": "{topic}",
            "questions": [
                ...
                "correct_answer": "A. Option A" // Or "B. Option B", "C. Option C", "D. Option D"
                }},
